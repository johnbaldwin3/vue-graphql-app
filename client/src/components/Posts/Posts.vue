  ...
  <v-container fluid grid-list-xl>
    <v-layout row wrap v-if="infiniteScrollPosts">
      <v-flex xs12 sm6 v-for="post in infiniteScrollPosts.posts" :key="post._id">
        <v-card hover>
          <v-img @click.native="goToPost(post._id)" height="30vh" lazy :src="post.imageUrl"></v-img>
          <v-card-actions>
    ...
    <v-layout column>
      <v-flex xs12>
        <v-layout v-if="showMoreEnabled" justify-center row>
...
export default {
  ...
  methods: {
    ...
    showMorePosts() {
      this.pageNum += 1;
      //fetch more posts and transform original results
      this.$apollo.queries.infiniteScrollPosts.fetchMore({
        variables: {
          // pageNum has been incremented by 1
